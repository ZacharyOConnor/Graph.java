...
CMD_ARGS="$MAIN_CLASS $WORKER_CLASS -vif $INPUT_FORMAT_CLASS -vip $INPUT_GRAPH -vof $OUTPUT_FORMAT_CLASS $CUSTOM_OUTPUT -w 1 $ARGS"
echo "$0 $1 $2 $3 $4 $5 $6" >> SSPresults.txt

hdfs dfsadmin -safemode leave > hadoopSSPOutput.txt
hadoop fs -rm -r /user/zachary/$HADOOP_OUTPUT_DIR >> hadoopSSPOutput.txt


echo "hadoop jar $EXAMPLES_JAR $CMD_ARGS" > hadoopSSPOutput.txt
echo "hadoop jar $EXAMPLES_JAR $CMD_ARGS"
export HADOOP_HEAPSIZE=4096
START_TIME=`date`
hadoop jar $EXAMPLES_JAR $CMD_ARGS >> hadoopSSPOutput.txt 2>&1
END_TIME=`date`
